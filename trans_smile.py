...
def AC2BO(AC, atoms, charge, allow_charged_fragments=True, use_graph=True):
    ...
    removePair = []
    for i in range(len(unable_list)):
        for j in range(i):
            if AC[unable_list[i]][unable_list[j]] == 1:
                removePair.append((unable_list[i],unable_list[j]))

    for pair in removePair:
        AC[pair[0]][pair[1]] = 0
        AC[pair[1]][pair[0]] = 0
        if(pair[0] in unable_list): unable_list.remove(pair[0])
        if(pair[1] in unable_list): unable_list.remove(pair[1])

    AC_valence = list(AC.sum(axis=1))
    
#   remove bond of atom without pairs
    if(len(unable_list) > 0):
        for i in unable_list:
            maxValence = 0
            removeMark = 0
            for j in range(len( AC_valence)):
                if(maxValence < AC_valence[j] and AC[i][j] == 1):
                    maxValence = AC_valence[j]
                    removeMark = j
                AC[i][removeMark] = 0
                AC[removeMark][i] = 0
...
def xyz2AC_huckel(atomicNumList, xyz, charge):
    ...
    for i in range(num_atoms):
        for j in range(i+1,num_atoms):
            ...
            if pair_pop >= 0.35: #arbitry cutoff for bond. May need adjustment
                AC[i,j] = 1
                AC[j,i] = 1
...
def listInterface(Ele,Coor, look_for_charge=True):
    """
    [["C", [0.1, 0.2, 0.3] ],
     ["H", [0.1, 0.2, 0.5] ]]
    """
    ...
    num_atoms = len(Ele)
#   for atm in List:
#       atomic_symbols.append(atm[0])
#       xyz_coordinates.append(atm[1])
    atomic_symbols = Ele
    xyz_coordinates = Coor

    atoms = [int_atom(atom) for atom in atomic_symbols]

    return atoms, charge, title, xyz_coordinates  #########

def xyzfileToSMILE(Ele,coor):
    import argparse
    ...
    atoms, charge, title, xyz_coordinates = listInterface(Ele,coor) 
    use_huckel = True
    charge = 0
    # Get the molobjs
